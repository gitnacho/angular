...
Un enrutador se debe configurar con una lista de definiciones de ruta.

Cada definición se traduce en un objeto [`Route`](api/router/Route) que tiene dos cosas: a `path`, the URL path segment for this route; and a `component`, the component associated with this route.
...
<div class="alert is-important">
  ...
  **Nota**: The `RouterModule.forRoot()` method is a pattern used to register application-wide providers. Read more about application-wide providers in the [Singleton services](guide/singleton-services#forRoot-router) guide.
...
**Nota**: The guard can also tell the router to navigate elsewhere, effectively canceling the current navigation.
...
**Nota**: The observable provided to the `Router` must also complete. If the observable does not complete, the navigation does not continue.
...
### `CanActivate` ⏤ requerir autenticación

Las aplicaciones a menudo restringen el acceso a una determinada área en función de quién es el usuario.
Puedes permitir el acceso solo a usuarios autenticados o a usuarios con un rol específico.
Puedes bloquear o limitar el acceso hasta que se active la cuenta del usuario.

El protector `CanActivate` es la herramienta para administrar estas reglas de navegación.
