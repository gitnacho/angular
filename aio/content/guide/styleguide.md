...
Unos pocos términos son servicios desambiguos. They typically
indicate agency by ending in "-er". You may prefer to name
a service that logs messages `Logger` rather than `LoggerService`.
Decide si esta excepción es agradable en tu proyecto.
Como siempre, esfuérzate por la consistencia.
...
**Do** limit logic in a component to only that required for the view. Toda otra lógica se debe delegar a los servicios.
