...
3. HTTP Data Request. You can also get data from a server through an HTTP request. Por ejemplo:
...
4. MacroTasks, such as `setTimeout()` or `setInterval()`. You can also update the data in the callback function of a `macroTask` such as `setTimeout()`. Por ejemplo:
...
5. MicroTasks, such as `Promise.then()`. Other asynchronous APIs return a Promise object (such as `fetch`), so the `then()` callback function can also update the data. Por ejemplo:
