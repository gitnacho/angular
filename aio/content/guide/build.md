...
The base file `environment.ts`, contains the default environment settings. Por ejemplo:
...
If you need to access a backend that is not on `localhost`, set the `changeOrigin` option as well. Por ejemplo:
...
To help determine whether your proxy is working as intended, set the `logLevel` option. Por ejemplo:
