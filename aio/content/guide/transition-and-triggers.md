...
**Nota**: For this example, an element entering or leaving a view is equivalent to being inserted or removed from the DOM.
...
**Nota**: The following example uses `query()` and `stagger()` methods. For more information on these methods, see the [complex sequences](guide/complex-animation-sequences#complex-sequence) page.
...
**Nota**: Disabling animations application-wide is useful during end-to-end (E2E) testing.
</div>
