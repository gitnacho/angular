...
up with a project structure with a large number of relatively small files. Este es
...
regardless of whether it is in AngularJS or Angular. El
...
¡Felicidades! You're running a hybrid application! El
...
directiva. When you use the component from an Angular template,
...
template, the need may arise to *transclude* some content into it. Esta
...
**Nota**: The 'heroes' string inside the factory refers to the AngularJS `HeroesService`.
...
**Nota**: See the `LocationUpgradeConfig` for more configuration options available to the `LocationUpgradeModule.config()` method.
...
aplicación _{N}_. Switch to the [ngUpgrade bootstrap](#bootstrapping-hybrid-applications) method
instead.
...
The methods now return observables of type `PhoneData` and `PhoneData[]`. Este es
...
concerned with the internal structure of the application components. Ese
also means that, although you modify the project quite a bit during the upgrade, the E2E
test suite should keep passing with just minor modifications. Se
