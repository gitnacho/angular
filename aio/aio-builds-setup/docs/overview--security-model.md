# Descripción general ⏤ Modelo de seguridad


Siempre que se ejecuta una *SE* en *CircleCI*, queremos compilar `angular.io` y alojar los artefactos de compilación en
un servidor de acceso público para que los colaboradores (desarrolladores, diseñadores, autores, etc.) puedan obtener una vista previa
de los cambios sin tener que bajar y compilar la aplicación localmente.

Este documento analiza las consideraciones de seguridad asociadas con mover artefactos de compilación como
parte del proceso de *CI* y sirviéndolos públicamente.
...
1. **Receive notification from CircleCI of a completed build**
  ...
  *CircleCI* está configurado para activar un `webhook` en la vista previa de nuestro servidor cada vez que se completa una compilación.
  La carga útil contiene el número de compilación que se completó.
...
6. **If necessary update the corresponding PR's verification status**.
   ...
   whether it is publicly accessible or not), based on the new verification status. Por ejemplo, si
...
8. **Prevent overwriting previously deployed artifacts**.
   ...
   *Nota: A PR can contain multiple builds; one for each SHA that was built on CircleCI._
