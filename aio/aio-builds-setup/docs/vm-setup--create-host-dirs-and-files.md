...
**Nota**: The directories created inside that directory will be owned by user `www-data`.
...
- `preview-server-{prod,test,verify-setup}-*.log` ⏤
  ...
  - `-prod` ⏤ Archivos de "producción" (por ejemplo, durante el funcionamiento normal).
  - `-test` ⏤ Archivos "test" (p. ej., cuando se inicia una instancia de prueba con el comando
