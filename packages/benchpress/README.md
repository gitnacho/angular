...
<script>
  ...
  var complexHtmlString = '...'; // PENDIENTE
...
runner.sample({
  ...
  execute: () {
    fillBtn.click();
    // Nota: if fillBtn would use some asynchronous code,
...
by setting a numeric value on the `window` object. Por ejemplo:
...
- `requestCount`: ⏤ número de solicitudes enviadas desde el último inicio de navegación

Para recopilar estas métricas, necesitas los siguientes proveedores adicionales correspondientes:
